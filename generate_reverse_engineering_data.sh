...
PDFEOF
    ...
    echo '```asm' >>"$PDF_MD"
    grep -A 20 "LoRa" "${DATA_DIR}/full_disassembly.txt" | head -25 >>"$PDF_MD"
    echo '```' >>"$PDF_MD"
    echo >>"$PDF_MD"
...
PDFEOF
    ...
    # Copy LoRa image to data directory for PDF generation
    if [ -f "LoRa.jpeg" ]; then
        cp "LoRa.jpeg" "${DATA_DIR}/LoRa.jpeg"
        echo -e "${GREEN}Copying LoRa.jpeg for PDF generation...${NC}"
    fi # Generate PDF using pandoc
    if [ -f "${DATA_DIR}/LoRa.jpeg" ]; then
...
echo
if [ -f "Hacking_Embedded_LoRa.pdf" ]; then
    echo -e "${GREEN}📚 PDF Report: Hacking_Embedded_LoRa.pdf${NC}"
...
if [ -f "Hacking_Embedded_LoRa.pdf" ]; then
    echo -e "${BLUE}  Open PDF guide: open Hacking_Embedded_LoRa.pdf${NC}"
fi
echo
