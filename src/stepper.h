 ...
 * @author Kevin Thomas
 *
 * This header file provides a comprehensive interface/**
 * @brief Check if stepper interrupt flag is set
 *
/**
 * @file stepper.h
 * @brief ULN2003 28BYJ-48 4-Phase Stepper Motor Driver Interface
...
/**
 * @brief Set the global stepper interrupt flag to stop all operations
 *
 * This function sets an interrupt flag that causes all stepper operations
 * to stop immediately. The flag is checked during step operations to enable
 * immediate response to stop commands.
 */
void stepper_set_interrupt(void);

/**
 * @brief Clear the global stepper interrupt flag to allow operations
 *
 * This function clears the interrupt flag, allowing stepper operations
 * to proceed normally.
 */
...
/**
 * @brief Emergency stop all stepper motors immediately
 *
 * This function immediately disables all stepper motors by setting their GPIO pins low.
 * Used for emergency stop functionality when interrupt flag is set.
 *
 * @param motors Array of stepper motor structures
 * @param num_motors Number of motors in the array
 */
